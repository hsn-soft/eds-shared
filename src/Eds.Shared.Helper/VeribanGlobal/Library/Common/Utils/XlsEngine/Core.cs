...
namespace Eds.Shared.Helper.VeribanGlobal.Library.Common.Utils.XlsEngine
{
    ...
    {
        private readonly static Object thisLockFile = new Object();
        private readonly static Object thisLockFile2 = new Object();
    ...
    internal class XlsFat
    {
        ...
        public uint GetNextSector(uint sector)
        {
            if (m_fat.Count <= sector)
                throw new ArgumentOutOfRangeException("sector", "There's no such sector in FAT.");
            uint value = m_fat[(int)sector];
            if (value == (uint)FatMarkers.FAT_FatSector || value == (uint)FatMarkers.FAT_DifSector)
                throw new InvalidOperationException("Trying to read stream from FAT area.");
            return value;
        }
    ...
    internal class XlsRootDirectory
    {
        ...
        private readonly XlsDirectoryEntry m_root = null;
    ...
    internal class XlsDirectoryEntry
    {
        ...
        public XlsDirectoryEntry(byte[] bytes)
        {
            if (bytes.Length < Length)
                throw new InvalidDataException("Array is too small.");
            m_bytes = bytes;
        }
    ...
    internal class XlsBiffStream : XlsStream
    {
        ...
        public void Seek(int offset, SeekOrigin origin)
        {
            ...
            if (m_offset < 0)
                throw new IndexOutOfRangeException("Moving before stream start");
            if (m_offset > m_size)
                throw new IndexOutOfRangeException("Moving after stream end");
        }
    ...
    internal class XlsBiffRecord
    {
        ...
        protected XlsBiffRecord(byte[] bytes, uint offset)
        {
            if (bytes.Length - offset < 4)
                throw new InvalidDataException("Buffer size is less than minimum BIFF record size");
            m_bytes = bytes;
            m_readoffset = (int)(4 + offset);
            if (bytes.Length < offset + Size)
                throw new InvalidDataException("Buffer size is less than entry length.");
        }
        ...
        {
            uint _id = BitConverter.ToUInt16(bytes, (int)offset);
            switch ((BiffRecordType)_id)
            {
    ...
    internal class XlsBiffSst : XlsBiffRecord
    {
        ...
        public string GetString(uint SSTIndex)
        {
            if (SSTIndex < m_strings.Count)
                ...
                return "NOT FOUND #" + SSTIndex.ToString();
        }
    ...
    internal class XlsFormattedUnicodeString
    {
        ...
        [Flags]
        public enum FormattedUnicodes : byte
        {
            MultiByte = 0x01,
        ...
        /// String flags
        /// </summary>
        public FormattedUnicodes Flags
        {
            get { return (FormattedUnicodes)Buffer.GetByte(m_bytes, (int)m_offset + 2); }
        }
    ...
    internal class XlsBiffBoolErrCell : XlsBiffBlankCell
    {
        ...
        public bool Value
        {
            get { return base.ReadByte(0x6) != 0; }
        }
        ...
        public bool IsError
        {
            get { return base.ReadByte(0x7) != 0; }
        }
    ...
    internal class XlsBiffFormulaString : XlsBiffRecord
    {
        ...
        public string Value
        {
            get
            {
                ...
                {
                    byte[] tmpBytes = new byte[len * size];
                    for (int i = 0; i < len; ++i)
                    {
                        tmpBytes[i * size + 1] = 0;
                        tmpBytes[i * size + 0] = m_bytes[offset + i];
                    }
                    return UseEncoding.GetString(tmpBytes, 0, len * size);
                }
                else
                {
                    return UseEncoding.GetString(m_bytes, offset, len * size);
                }
            }
        }
    }
    ...
    {
        [Flags]
        public enum WindowTypes : ushort
        {
            Hidden = 0x1,
            Minimized = 0x2,
            //(Reserved) = 0x4,
        ...
        /// </summary>
        public WindowTypes Flags
        {
            get { return (WindowTypes)base.ReadUInt16(0x8); }
        }
    ...
    internal class XlsWorkbookGlobals
    {
        ...
        public List<XlsBiffRecord> Fonts { get; private set; } = new List<XlsBiffRecord>();

        public List<XlsBiffRecord> Formats { get; private set; } = new List<XlsBiffRecord>();

        public List<XlsBiffRecord> ExtendedFormats { get; private set; } = new List<XlsBiffRecord>();

        public List<XlsBiffRecord> Styles { get; private set; } = new List<XlsBiffRecord>();

        public List<XlsBiffBoundSheet> Sheets { get; private set; } = new List<XlsBiffBoundSheet>();
    ...
    internal class XlsWorksheet
    {
        ...
        private readonly uint m_dataOffset = 0;
